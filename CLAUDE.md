...
1. `git tag vX.Y.Z && git push --tags` → GitHub Actions が universal dmg をビルドして Release 公開
2. `scripts/update-cask.sh vX.Y.Z` → Love-Rox/homebrew-tap の cask を新バージョンへ更新
3. アプリ内更新チェック(updater.rs、起動20秒後+24時間ごと)が新リリースを検知してバナー通知。更新手段は OS で分岐: macOS = `brew upgrade` へ誘導(無署名 Homebrew 配布のため自己置換しない)、Windows/Linux = tauri-plugin-updater で自己更新(minisign 署名。秘密鍵は `~/.tauri/harushion.key`、CI は TAURI_SIGNING_PRIVATE_KEY secret。紛失すると既存ユーザーへ更新を配れなくなるので注意)
4. ビルド番号 = コミット数(build.rs が Info.plist の CFBundleVersion へ自動埋め込み)
